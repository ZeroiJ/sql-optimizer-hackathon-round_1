...
try:
    from sql_optimizer_env.models import SQLAction, SQLObservation, SQLReward
except ImportError:
    from sql_optimizer_env.models import SQLAction, SQLObservation, SQLReward
...
def _seed_data(conn: sqlite3.Connection) -> None:
    ...
    review_texts = [
        "Great product!", "Not bad", "Could be better", "Excellent", "Terrible quality",
    ]
    for i in range(100):
        cur.execute(
    ...
    for i in range(600):
        cur.execute(
            ...
            (
                random.randint(1, 300), random.randint(1, 50),
                random.randint(1, 10), round(random.uniform(5, 200), 2),
            ),
        )
    for i in range(200):
        cur.execute(
            ...
            (
                ...
                random.randint(1, 5), random.choice(review_texts),
            ),
        )
    conn.commit()


class SQLOptimizerEnv(Environment):
    ...
    def _has_cte_or_window(self, query: str) -> bool:
        upper = query.upper()
        return "WITH " in upper or "OVER(" in upper.replace(" ", "") or "OVER (" in upper

    def reset(
        self, task_id: Optional[str] = None, seed: Optional[int] = None, episode_id: Optional[str] = None, **kwargs: Any
    ) -> SQLObservation:
        if seed is not None:
            random.seed(seed)
    ...
    def step(self, action: SQLAction, timeout_s: Optional[float] = None, **kwargs: Any) -> SQLObservation:
        ...
        if not semantic_match:
            # -500 penalty for semantic mismatches
            final_value = -500.0
            done = self._attempts >= self._max_attempts
            ...
            return SQLObservation(
                ...
                schema_description=SCHEMA_DESCRIPTION,
                broken_query=task["broken_query"],
                error_message="Semantic mismatch penalty.",
                current_score=self._current_score,
                attempts=self._attempts,
                max_attempts=self._max_attempts,
                done=done,

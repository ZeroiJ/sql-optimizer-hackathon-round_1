"""Gradio demo dashboard for the Autonomic DBRE agent."""
...
from sql_optimizer_env.models import RewriteQueryAction
from sql_optimizer_env.server.sql_optimizer_environment import (
    SQLOptimizerEnvironment,
    _explain_analyze_json,
)


def _dummy_agent_inference(broken_query: str) -> RewriteQueryAction:
    """Mock agent action for demo while training is in progress."""
...
def run_demo_episode() -> tuple[str, str, str, str, str, str, str]:
    """Run one reset+step demo episode and return UI values."""
    ...
    try:
        ...
        new_ms = float(
            info.get(
                "new_execution_time_ms",
                info.get("initial_execution_time_ms", baseline_ms),
            )
        )
        ...
        sql_diff = _format_sql_diff(broken_query, action.new_sql)
        return (
            broken_query,
            schema_alerts,
...
def build_dashboard() -> gr.Blocks:
    """Create the hackathon demo dashboard."""
    with gr.Blocks(theme=gr.Theme.from_hub("gradio/monochrome")) as demo:
        ...
        run_button = gr.Button(
            "Inject Database Chaos & Run Agent",
            variant="primary",
            size="lg",
        )
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                reward_breakdown = gr.Code(
                    label="Multi-Dimensional Reward",
                    language="json",
                )
